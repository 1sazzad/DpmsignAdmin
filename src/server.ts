...
const initializeServer = async (): Promise<void> => {
  try {
    ...
    try {
      server.listen(port, () => {
        console.log(
          `Server is running on ${serverBaseUrl} in ${nodeEnv} mode`.yellow
        );
        console.log(`API Docs: ${urlJoin(serverBaseUrl, apiDocsUrl)}`.blue);
      });
    } catch (err: Error | any) {
      console.log("Error occurred starting the server: ".red, err.message);
    }
  } catch (err: Error | any) {
